    ...
    try:
        ...
        for idx, face in enumerate(faces):
            ...
            detected_faces.append({
                "index": idx,
                "name": best_name,
                # Cast to native Python float to avoid JSON serialization issues with numpy.float32
        ...
        return JSONResponse(content={
            ...
            "faces": [{"index": i, 
                      "name": face["name"], 
                      # Ensure distance is a native float and guard against nan/inf
                      "distance": float(face["distance"]) if not (math.isnan(face["distance"]) or math.isinf(face["distance"])) else 999.0,
                      "bbox": face["bbox"]} 
